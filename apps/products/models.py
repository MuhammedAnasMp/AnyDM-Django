...
class Product(models.Model):
    ...
    weight = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.0, help_text="Weight in kg")
    dimensions = models.CharField(
        max_length=100, blank=True, null=True, help_text="L x W x H in cm")
...
def delete_from_cloudinary(public_id, resource_type="image"):
    ...
    cloud_name = getattr(settings, 'CLOUDINARY_CLOUD_NAME',
                         os.environ.get('CLOUDINARY_CLOUD_NAME', 'dx5bqewfx'))
    ...
    if not all([cloud_name, api_key, api_secret]):
        logger.warning(
            "Cloudinary credentials not fully configured. Skipping deletion of %s", public_id)
        return False
    ...
    try:
        ...
        if result.get("result") == "ok":
            logger.info("Successfully deleted %s (%s) from Cloudinary",
                        public_id, resource_type)
            return True
        else:
            logger.error("Failed to delete %s from Cloudinary: %s",
                         public_id, result)
            return False
    except Exception as e:
        logger.error(
            "Error calling Cloudinary destroy API for %s: %s", public_id, e)
        return False
...
def get_public_id_and_type(instance):
    ...
    url = getattr(instance, 'media_url', None) or getattr(
        instance, 'main_media_url', None)
    if url and "res.cloudinary.com" in url:
        try:
            ...
            if len(parts) > 1:
                ...
                resource_type = "video" if getattr(
                    instance, 'media_type', None) == "VIDEO" else "image"
                return public_id, resource_type
        except Exception:
            pass
