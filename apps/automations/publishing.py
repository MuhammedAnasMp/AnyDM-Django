...
def create_media_container(account, post_type, media_url, caption="", cover_url=None, share_to_feed=True, carousel_urls=None):
    ...
    if not access_token or not instagram_user_id:
        raise ValueError(
            f"Account {account.id} missing access token or Instagram user ID.")
    ...
    if post_type in ["REELS", "VIDEO"] and not is_media_video:
        logger.warning(
            f"[PUBLISHER] post_type was '{post_type}' but media URL is an image. Switching post_type to 'IMAGE'.")
        post_type = "IMAGE"
    elif post_type == "IMAGE" and is_media_video:
        logger.warning(
            f"[PUBLISHER] post_type was 'IMAGE' but media URL is a video. Switching post_type to 'REELS'.")
        post_type = "REELS"
    ...
    if post_type == "IMAGE":
        ...
        is_video = is_media_video or media_url.lower().endswith(
            ('.mp4', '.mov', '.avi', '.webm')) or 'video' in media_url
        data = {
            "media_type": "STORIES",
        ...
        for item_url in valid_items:
            is_vid = any(item_url.lower().endswith(ext) for ext in [
                         '.mp4', '.mov', '.avi', '.webm', '.mkv']) or '/video/upload/' in item_url
            child_data = {
            ...
            child_res = requests.post(
                url, params=params, data=child_data, timeout=30)
            child_json = child_res.json()
            if "id" not in child_json:
                error_detail = child_json.get("error", {}).get(
                    "message", str(child_json))
                raise Exception(
                    f"Failed to create carousel item for {item_url}: {error_detail}")
    ...
    logger.info(
        f"[PUBLISHER] Creating container for {post_type} on @{account.username}")
...
def poll_container_status(account, container_id, max_attempts=40, delay_seconds=3):
    ...
    for attempt in range(max_attempts):
        ...
        logger.info(
            f"[PUBLISHER] Polling container {container_id} (Attempt {attempt+1}/{max_attempts}): {status_code} - Data: {stat_data}")

        if status_code == "FINISHED":
            return True
        elif status_code == "ERROR":
            err = stat_data.get("status") or stat_data.get("error", {}).get(
                "message") or "Container processing failed on Meta."
            raise Exception(f"Meta container error: {err}")
        elif status_code == "EXPIRED":
            raise Exception("Meta container creation expired.")

    raise TimeoutError(
        f"Container processing timed out for ID {container_id} after {max_attempts * delay_seconds}s.")
...
def publish_container(account, container_id, max_retries=6, retry_delay=3):
    ...
    for attempt in range(max_retries):
        logger.info(
            f"[PUBLISHER] Publishing container {container_id} for @{account.username} (Attempt {attempt+1}/{max_retries})")
        ...
        if "id" in res_data:
            ...
            logger.info(
                f"[PUBLISHER] Successfully published to Instagram! Media ID: {media_id}")
            return media_id
        ...
        if any(keyword in error_msg.lower() for keyword in ["not available", "not ready", "in progress", "loading"]):
            logger.warning(
                f"[PUBLISHER] Container {container_id} not ready yet ({error_msg}). Retrying in {retry_delay}s...")
    ...
    raise Exception(
        f"Publishing failed: Container {container_id} was not ready after {max_retries * retry_delay}s.")
...
def fetch_media_permalink(account, media_id):
    ...
    try:
        ...
        params = {"access_token": access_token,
                  "fields": "permalink,media_type,thumbnail_url"}
        res = requests.get(url, params=params, timeout=15)
        res_data = res.json()
        return res_data.get("permalink")
    except Exception as e:
        logger.warning(
            f"[PUBLISHER] Failed to fetch permalink for media {media_id}: {e}")
        return None
...
def broadcast_scheduled_post_update(scheduled_post, event_action="status_update"):
    ...
    try:
        ...
        logger.info(
            f"[WS-BROADCAST] Post #{scheduled_post.id} status '{scheduled_post.status}' broadcasted via WebSocket.")
    except Exception as ws_err:
        logger.warning(
            f"[WS-BROADCAST] Failed to broadcast WebSocket update for post #{scheduled_post.id}: {ws_err}")
...
def execute_publish_post(scheduled_post):
    ...
    try:
        ...
        is_vid = any(scheduled_post.media_url.lower().endswith(ext) for ext in [
                     '.mp4', '.mov', '.avi', '.webm', '.mkv']) or '/video/upload/' in scheduled_post.media_url
        if scheduled_post.post_type in ["REELS", "VIDEO"] or (scheduled_post.post_type == "STORIES" and is_vid):
            poll_container_status(account, container_id,
                                  max_attempts=45, delay_seconds=4)
        else:
        ...
        if scheduled_post.product:
            try:
                ...
                prod.save(update_fields=['media_id',
                          'instagram_permalink', 'source_id'])
                logger.info(
                    f"[PUBLISHER] Linked Product #{prod.id} with published Instagram media #{media_id}")
            except Exception as prod_err:
                logger.error(
                    f"[PUBLISHER] Error syncing Product #{scheduled_post.product.id}: {prod_err}")
        ...
        if auto_cfg and auto_cfg.get('enabled', True):
            try:
                ...
                target_media_type = 'reel' if scheduled_post.post_type in ['REELS', 'VIDEO'] else (
                    'story' if scheduled_post.post_type == 'STORIES' else 'post')
                custom_cover = scheduled_post.cover_url
                rule_name = auto_cfg.get(
                    'rule_name') or f"Auto DM for {scheduled_post.product.title if scheduled_post.product else 'Post'}"

                raw_keywords = auto_cfg.get(
                    'keywords', ['PRICE', 'BUY', 'LINK'])
                if isinstance(raw_keywords, str):
                    keywords = [k.strip().upper()
                                for k in raw_keywords.split(',') if k.strip()]
                ...
                follower_gate_enabled = bool(
                    auto_cfg.get('follower_gate', False))
                raw_fg_msgs = auto_cfg.get('follower_gate_messages', [
                                           "Please follow our page to unlock this offer! ✨"])
                follower_gate_messages = raw_fg_msgs if isinstance(
                    raw_fg_msgs, list) else [str(raw_fg_msgs)]
                ...
                if scheduled_post.product and dm_format == 'generic_template':
                    prod = scheduled_post.product
                    store_link = f"https://zoyee.in/{account.username}/product/{prod.id}"

                    # Ensure a valid image URL is used for Instagram Generic Template card
                    card_img = auto_cfg.get('card_image_url') or custom_cover or getattr(
                        prod, 'thumbnail_url', None)
                    if not card_img:
                        img_media = prod.gallery.filter(
                            media_type='IMAGE').first()
                        if img_media and img_media.media_url:
                            ...
                            card_img = prod.main_media_url or "https://zoyee.in/favicon.ico"

                    price_str = f"{prod.currency or '₹'}{prod.price:g}" if isinstance(
                        prod.price, (int, float)) else f"{prod.currency or '₹'}{prod.price}"
                ...
                raw_replies = auto_cfg.get('comment_replies') or [auto_cfg.get('comment_reply')] or [
                    "Sent you a DM with the product link! 🛍️ Check your inbox!"]
                if isinstance(raw_replies, str):
                    comment_replies = [r.strip()
                                       for r in raw_replies.split('\n') if r.strip()]
                elif isinstance(raw_replies, list):
                    comment_replies = [str(r).strip()
                                       for r in raw_replies if str(r).strip()]
                else:
                    comment_replies = [
                        "Sent you a DM with the product link! 🛍️ Check your inbox!"]
                ...
                try:
                    ...
                    logger.warning(
                        f"[PUBLISHER] Non-critical: Failed to pre-generate visual_data for rule #{rule.id}: {ve}")

                logger.info(
                    f"[PUBLISHER] Successfully auto-created AutomationRule #{rule.id} ('{rule_name}') targeting media #{media_id}")
            except Exception as auto_err:
                logger.error(
                    f"[PUBLISHER] Error creating auto-DM rule for post #{scheduled_post.id}: {auto_err}")

        # Real-time WebSocket broadcast
        broadcast_scheduled_post_update(scheduled_post, "published")

        logger.info(
            f"[PUBLISHER] Post #{scheduled_post.id} completed successfully! Link: {permalink}")
        return True, permalink

    except Exception as e:
        error_msg = str(e)
        logger.error(
            f"[PUBLISHER] Execution failed for post #{scheduled_post.id}: {error_msg}")
