...
def send_instagram_dm(account, recipient_id, message_data, dm_format="text", recipient_type="id"):
    ...
    if not access_token or not instagram_user_id:
        logger.error(f"Cannot send DM: Account {account.id} missing access token or Instagram user ID.")
        return False, "Missing credentials"

    url = f"https://graph.instagram.com/v25.0/{instagram_user_id}/messages"
    headers = {
    ...
    if dm_format == "text":
        ...
        message_payload = {
            "attachment": {
                "type": message_data.get("attachment_type", "image"),
                "payload": {
...
def execute_automation(interaction):
    ...
    # Fetch active rules for this seller
    ...
    for rule in rules:
        ...
        if rule.rule_type in ['comment_automation', 'giveaway_comment', 'product_inquiry_comment']:
            ...
            # DM or CLICK (postback buttons/quick replies), but NOT a story reply
